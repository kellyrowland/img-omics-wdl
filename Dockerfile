...
RUN apt-get install curl gcc make emacs -y
...
RUN tarfile=omics.20201026.1334.tar.gz && cd /opt && /bin/bash -c "source /miniconda3/etc/profile.d/conda.sh && conda activate && wget http://portal.nersc.gov/dna/metagenome/assembly/img-scripts/$tarfile && tar -zxvf $tarfile && chmod -R 755 omics && rm $tarfile"
