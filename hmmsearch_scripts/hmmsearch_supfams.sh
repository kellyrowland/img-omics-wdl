...
then
    ...
    rm $(ls $tmp_dir/*supfam* | grep -v log) &> /dev/null
    ...
    cat $proteins_fasta | parallel --pipe --recstart '>' \
    		                ...
    		                $supfam_hmm_db $tmp_dir'/tmp.$$.split.faa 1> /dev/null;'
    exit_code=$?
    if [[ $exit_code -ne 0 ]]
    then
    ...
    cat $tmp_dir/tmp.supfam.* > ${proteins_fasta%.*}.supfam.domtblout
    exit_code=$?
    if [[ $exit_code -ne 0 ]]
    then
    ...
    hmmsearch_cmd="$hmmsearch_cmd --domtblout ${proteins_fasta%.*}.supfam.domtblout "
    hmmsearch_cmd="$hmmsearch_cmd $supfam_hmm_db $proteins_fasta"
