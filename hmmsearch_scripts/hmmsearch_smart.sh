...
if [[ $# -lt 2 || $# -gt 8 ]]
then
	printf "Usage: $(basename $0) <proteins_fasta> <smart_hmm_db> " >&2
...
if [[ ! -f $smart_hmm_db ]]
then
	echo "$smart_hmm_db is not a file! Aborting!" >&2
	exit 1
fi
...
then
    ...
    rm $(ls $tmp_dir/*smart* | grep -v log) &> /dev/null
    ...
    cat $proteins_fasta | parallel --pipe --recstart '>' \
    		                ...
    		                $smart_hmm_db $tmp_dir'/tmp.$$.split.faa 1> /dev/null;'
    exit_code=$?
    if [[ $exit_code -ne 0 ]]
    then
    ...
    cat $tmp_dir/tmp.smart.* > ${proteins_fasta%.*}.smart.domtblout
    exit_code=$?
    if [[ $exit_code -ne 0 ]]
    then
    ...
    hmmsearch_cmd="$hmmsearch_cmd --domtblout ${proteins_fasta%.*}.smart.domtblout "
    hmmsearch_cmd="$hmmsearch_cmd $smart_hmm_db $proteins_fasta"
...
smart_gff=${proteins_fasta%_*}_smart.gff
grep -v '^#' ${proteins_fasta%.*}.smart.domtblout | \
...
echo "$(date +%F_%T) - Done with SMART annotation."
