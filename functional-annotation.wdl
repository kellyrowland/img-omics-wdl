...
task smart {
  ...
  command <<<
    ...
    then
        ...
        cat ${input_fasta} | parallel --pipe --recstart '>' --blocksize $blocksize 'cat > '$tmp_dir'/tmp.$$.split.faa;'$hmmsearch_base_cmd' --domtblout '$tmp_dir'/tmp.smart.$$.domtblout ${smart_db} '$tmp_dir'/tmp.$$.split.faa 1> /dev/null;'
...
task cog {
  ...
  command <<<
    ...
    then
        ...
        # Use parallel to split up the input and
        # run hmmsearch in parallel on those splits
        cat ${input_fasta} | parallel --pipe --recstart '>' --blocksize $blocksize 'cat > '$tmp_dir'/tmp.$$.split.faa;'$hmmsearch_base_cmd' --domtblout '$tmp_dir'/tmp.cog.$$.domtblout ${cog_db} '$tmp_dir'/tmp.$$.split.faa 1> /dev/null;'
  ...
  command <<<
    if [[ ${threads} -gt ${par_hmm_inst} ]]
      then
          ...
          # Use parallel to split up the input and
          # run hmmsearch in parallel on those splits
	  cat ${input_fasta} | parallel --pipe --recstart '>' --blocksize $blocksize 'cat > '$tmp_dir'/tmp.$$.split.faa;'$hmmsearch_base_cmd' --domtblout '$tmp_dir'/tmp.tigrfam.$$.domtblout ${tigrfam_db} '$tmp_dir'/tmp.$$.split.faa 1> /dev/null;'	  
#          cat ${input_fasta} | parallel --pipe --recstart '>' \
#                               --blocksize $blocksize \
#                               cat > $tmp_dir/tmp.$$.split.faa;  \
#                               $hmmsearch_base_cmd \
  ...
  command <<<
    if [[ ${threads} -gt ${par_hmm_inst} ]]
      then
          ...
          # Use parallel to split up the input and
          # run hmmsearch in parallel on those splits
	  cat ${input_fasta} | parallel --pipe --recstart '>' --blocksize $blocksize 'cat > '$tmp_dir'/tmp.$$.split.faa;'$hmmsearch_base_cmd' --domtblout '$tmp_dir'/tmp.superfam.$$.domtblout ${superfam_db} '$tmp_dir'/tmp.$$.split.faa 1> /dev/null;'	  
#          cat ${input_fasta} |  parallel --pipe --recstart '>' \
#                               --blocksize $blocksize \
#                               cat > $tmp_dir/tmp.$$.split.faa;  \
#                               $hmmsearch_base_cmd \
...
task pfam {
  ...
  command <<<
    ...
    then
        ...
        # Use parallel to split up the input and
        # run hmmsearch in parallel on those splits
        cat ${input_fasta} | parallel --pipe --recstart '>' --blocksize $blocksize 'cat > '$tmp_dir'/tmp.$$.split.faa;'$hmmsearch_base_cmd' --domtblout '$tmp_dir'/tmp.pfam.$$.domtblout ${pfam_db} '$tmp_dir'/tmp.$$.split.faa 1> /dev/null;'	
#        cat ${input_fasta} | parallel --pipe --recstart '>' \
#                             --blocksize $blocksize \
#                             cat > $tmp_dir/tmp.$$.split.faa;  \
#                             $hmmsearch_base_cmd \
...
task cath_funfam {
  ...
  command <<<
    ...
    then
        ...
        # Use parallel to split up the input and
        # run hmmsearch in parallel on those splits
        cat ${input_fasta} | parallel --pipe --recstart '>' --blocksize $blocksize 'cat > '$tmp_dir'/tmp.$$.split.faa;'$hmmsearch_base_cmd' --domtblout '$tmp_dir'/tmp.cath_funfam.$$.domtblout ${cath_funfam_db} '$tmp_dir'/tmp.$$.split.faa 1> /dev/null;'		
#        cat ${input_fasta} | parallel --pipe --recstart '>' \
#                             --blocksize $blocksize \
#                             cat > $tmp_dir/tmp.$$.split.faa;  \
#                             $hmmsearch_base_cmd \
